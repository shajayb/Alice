...
main(int argc, char *argv[])
{
	...
	Py_Initialize();
	{
		...
		////py::eval_file("funcTest.py", scope);
		...
		py::object scope1 = py::module::import("funcTest").attr("forceD");
		
		py::module res = scope1.cast<py::module>();

		py::object result_py = res.call(3, 5);
	...
	//pName = PyString_FromString(argv[1]); ;//3.4:// PyUnicode_DecodeFSDefault(argv[1]);
	...
	//		//	py::object result_py(pValue,false);
	//		//	py::array res = result_py.cast<py::array>();;
	//	

	//			//py::buffer_info info_res = res.request();
	//		/*	if ((info_res.ndim != 1))
	//				std::cout << "Number of dimensions must be one" << std::endl;*/
	...
	//			//for (auto c:ret)printf("%1.2f,\n", c);
	//			
	//			//std::cout << info_res.shape[0] << " -------- s[]" << std::endl;
